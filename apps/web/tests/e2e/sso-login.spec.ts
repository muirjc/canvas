...
// Keycloak* (docker compose --profile sso up -d keycloak) with the API server started with
// OIDC_ISSUER_URL/CLIENT_ID/CLIENT_SECRET/REDIRECT_URI pointed at it (see RUNBOOK.md's
// "Keycloak SSO" section), which is not part of the default `npm run dev` flow every other spec
// assumes. canvas-v4u: CI's e2e-tests job (.github/workflows/ci.yml) now stands up the identical
// docker-compose.yml keycloak service and sets these, so this spec does run there.
